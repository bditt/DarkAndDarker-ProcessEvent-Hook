#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: BP_GoblinWarrior_Nightmare

#include "Basic.hpp"

#include "BP_GoblinWarrior_Nightmare_classes.hpp"
#include "BP_GoblinWarrior_Nightmare_parameters.hpp"


namespace SDK
{

// Function BP_GoblinWarrior_Nightmare.BP_GoblinWarrior_Nightmare_C.ExecuteUbergraph_BP_GoblinWarrior_Nightmare
// (Final, UbergraphFunction, HasDefaults)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void ABP_GoblinWarrior_Nightmare_C::ExecuteUbergraph_BP_GoblinWarrior_Nightmare(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_GoblinWarrior_Nightmare_C", "ExecuteUbergraph_BP_GoblinWarrior_Nightmare");

	Params::BP_GoblinWarrior_Nightmare_C_ExecuteUbergraph_BP_GoblinWarrior_Nightmare Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_GoblinWarrior_Nightmare.BP_GoblinWarrior_Nightmare_C.OnStuckByShield
// (Event, Public, BlueprintEvent)
// Parameters:
// class ADCCharacterBase*                 InInstigator                                           (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)
// class AActor*                           InEffectCauser                                         (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)

void ABP_GoblinWarrior_Nightmare_C::OnStuckByShield(class ADCCharacterBase* InInstigator, class AActor* InEffectCauser)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_GoblinWarrior_Nightmare_C", "OnStuckByShield");

	Params::BP_GoblinWarrior_Nightmare_C_OnStuckByShield Parms{};

	Parms.InInstigator = InInstigator;
	Parms.InEffectCauser = InEffectCauser;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_GoblinWarrior_Nightmare.BP_GoblinWarrior_Nightmare_C.GameplayTagUpdated
// (Event, Protected, BlueprintEvent)
// Parameters:
// struct FGameplayTag                     InGameplayTag                                          (ConstParm, BlueprintVisible, BlueprintReadOnly, Parm, NoDestructor, HasGetValueTypeHash)
// int32                                   InCount                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void ABP_GoblinWarrior_Nightmare_C::GameplayTagUpdated(const struct FGameplayTag& InGameplayTag, int32 InCount)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_GoblinWarrior_Nightmare_C", "GameplayTagUpdated");

	Params::BP_GoblinWarrior_Nightmare_C_GameplayTagUpdated Parms{};

	Parms.InGameplayTag = std::move(InGameplayTag);
	Parms.InCount = InCount;

	UObject::ProcessEvent(Func, &Parms);
}

}

